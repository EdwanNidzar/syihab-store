    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            @forelse ($products as $product)
                <div class="bg-white rounded-lg shadow hover:shadow-md transition p-4 flex flex-col product-card relative">
                    <!-- Badge Pre-Order (Conditional) -->
                    @if(empty($product->variations))
                    <div class="absolute top-2 right-2 z-10">
                        <div class="bg-purple-100 w-14 h-14 rounded-full flex items-center justify-center relative drop-shadow-lg">
                    ...
                    @if(empty($product->variations))
                    <div class="mb-2">
                        <span class="inline-flex items-center gap-1 bg-purple-100 text-purple-800 text-xs font-medium px-2.5 py-0.5 rounded-full">
                    ...
                    @if(!empty($product->variations))
                        <a href="https://wa.me/628115546464?text={{ urlencode('Halo admin, saya ingin memesan ' . $product->name) }}"
                           target="_blank"
                           class="block text-center bg-gradient-to-r from-green-500 to-green-600 hover:from-green-600 hover:to-green-700 text-white font-semibold py-3 rounded-lg transition duration-200 shadow-md mt-4">
                            Pesan Sekarang
                        </a>
                    @else
                        <a href="https://wa.me/628115546464?text={{ urlencode('Halo admin, saya ingin Pre-Order ' . $product->name) }}"
                           target="_blank"
                           class="block text-center bg-gradient-to-r from-purple-500 to-purple-600 hover:from-purple-600 hover:to-purple-700 text-white font-semibold py-3 rounded-lg transition duration-200 shadow-md mt-4">
                            Pre-Order
                        </a>
                    @endif
                </div>
