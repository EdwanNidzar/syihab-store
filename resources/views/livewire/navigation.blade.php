<nav class="bg-white shadow-lg sticky top-0 z-50" x-data="{ isMobileSearchOpen: false, isMobileMenuOpen: false }">
    ...
    <!-- Mobile Version - Modified -->
    <div class="md:hidden fixed top-0 left-0 right-0 bg-white shadow z-50">
        <div class="container mx-auto px-4">
            <div class="flex items-center h-16">
                <!-- Logo Mobile - Only Image -->
                <a href="/" class="flex items-center mr-2">
                    <img src="{{ asset('img/logo/new-logo-syihab.png') }}" alt="Syihab Store Logo" class="h-8 w-auto rounded-full">
                </a>
                ...
                <div class="flex-1 mx-2">
                    <div class="relative">
                        <input wire:model.live.500ms.debounce.500ms="search" type="text"
                            ...
                            class="w-full px-3 py-2 text-sm rounded-full border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500"
                            wire:focus="showDropdown = true" wire:keydown.escape="showDropdown = false">
                        <div wire:loading class="absolute right-3 top-2">
                ...
                <!-- Menu Toggle -->
                <button @click="isMobileMenuOpen = !isMobileMenuOpen" class="p-2 text-gray-500">
        ...
        @if ($showDropdown && strlen($search) >= 2)
            <div class="absolute top-16 left-0 right-0 bg-white border-t z-40 max-h-[60vh] overflow-y-auto shadow-lg">
                @if ($productResults->isNotEmpty())
                    ...
                    @foreach ($brandResults as $brand)
                        <a href="{{ route('brand-detail', ['slug' => $brand->slug]) }}"
                            ...
                            <div class="flex items-center space-x-3">
                                <div class="flex-shrink-0">
                                    @if ($brand->logo)
                                        <div class="w-14 h-14 bg-white rounded-full flex items-center justify-center overflow-hidden border border-gray-200">
                                            <img src="{{ asset('storage/' . $brand->logo) }}"
                                                class="max-w-full max-h-full object-contain p-1">
                                        </div>
                                    @else
                                        <div class="w-14 h-14 rounded-full bg-gradient-to-br from-blue-50 to-gray-100 flex items-center justify-center">
                                            <span class="text-sm font-medium text-gray-600">{{ substr($brand->name, 0, 2) }}</span>
                                        </div>
                                    @endif
                                </div>
                                <div>
                                    <p class="font-medium">{{ $brand->name }}</p>
                                    <p class="text-xs text-gray-500">{{ $brand->products_count ?? 0 }} produk tersedia</p>
                                </div>
                            </div>
                        </a>
                    @endforeach
        ...
        <!-- Mobile Menu Panel -->
        <div x-show="isMobileMenuOpen" x-transition class="absolute top-16 left-0 right-0 bg-white border-t z-40 shadow-lg">
            <div class="container mx-auto px-4 py-2">
                ...
                <a href="{{ route("events") }}" class="block py-3 border-b text-gray-800">Events</a>
                <a href="{{ route("credits") }}" class="block py-3 border-b text-gray-800">Simulasi Kredit</a>
